...
### Phase 2.1 — 명령 유실 수정 · CAN 물리층 감시 (2026-09-03)

증상은 "첫 명령을 보내도 프레임이 안 나간다"였고, **로그가 한 줄도 남지 않아** 원인 특정이
불가능했다. 세션 3개(PID 5519/9270/9383) 모두 WARN·ERROR 0줄.

- **원인 1 — `ros2 topic pub -1` 이 메시지를 조용히 버린다.**
  `ros2topic/verb/pub.py:189,318` — `--keep-alive` 기본값이 **0.1초**다. 방금 만든 RELIABLE
  퍼블리셔가 RTPS 핸드셰이크를 끝내기 전에 노드가 파괴된다. `get_subscription_count()`(같은 파일
  `:281`)는 **discovery 매칭만** 뜻하고 전달 보장이 아니다.
  실측 **70회 중 1회 유실**(exit 0, "발행함" 출력까지 정상, 노드 쪽 로그 0줄).
  → `scripts/ak45_deg` 를 `pub -t 3 -r 10 --keep-alive 1.0` 으로 변경. 노드는 같은 목표값을
    래치만 하므로 중복 발행은 **멱등**하다.
- **원인 2 — 노드가 성공/실패를 아무것도 알려주지 않았다.**
  `onCommand()` 는 수락 시 무로그, `onCommandTimer()` 의 S3 게이트(`valid==0 || !wd_ok`)는
  **완전 무로그 skip** 이었다. 사람 눈에는 두 경우가 똑같이 "모터가 안 움직임"으로만 보인다.
  → `onCommand()` 에 목표각 수신 INFO(첫 명령은 무조건, 이후 1초 스로틀),
    S3 게이트에 WARN 추가.
- **원인 3(별건, 미해결) — CAN 물리층 신호 품질 불량.** 아래 P16.
  → 라이브러리에 **`CAN_RAW_ERR_FILTER` 활성화**. 기본값이 0 이라 그동안 **bus-off 조차**
    소켓에 배달되지 않았다. `BusStatus` / `ak45_get_bus_status()` / `ak45_bus_state_str()` 신설,
    `/diagnostics` 에 **`ak45/can_bus` 항목** 추가(status 6개 → **7개**).

| 검증 | 결과 |
|---|---|
| 피드백 안정성 60초 | 정확히 **50.0Hz**, 최대 갭 **21.75ms**, **워치독 트립 0회** |
| `ak45_deg` 10회 연속 | 목표각 수신 로그 **11줄**(첫 명령 1 + 이후 10), 유실 **0** |
| 명령 프레임 | `00000402 [4] 00 01 86 A0` = 10.0000도, 10Hz |
| `can_bus` 진단 | `error-warning`, bit-stuffing 725, form 87, **no-ack 0**, bus-off 0 |
| 빌드 | colcon **경고 0**, 루트 `make` **경고 0** (양쪽 표준 P15 유지) |

⚠️ 루트의 `ak45_36_socketcan_control.{h,cpp}` 는 패키지 사본과 **바이트 단위로 동일해야 한다**
(스탠드얼론 Makefile 빌드가 루트 사본을 쓴다). 이번 수정도 양쪽에 반영했다.
...
- [ ] **CAN `restart-ms` 설정** (P16) — 현재 `restart-ms 0` 이라 bus-off 가 나면 **자동 복구되지
      않는다.** 모터 6개로 늘리면 에러 노출이 6배가 되므로 그 전에 반드시 켠다:
      `sudo ip link set can0 down && sudo ip link set can0 up type can bitrate 1000000 restart-ms 100`
- [ ] **CAN 종단저항·배선 점검** (P16) — bit-stuffing/form 에러가 **초당 약 20회**. 코드로 못 고친다.
      양단 120Ω, 스터브 길이, CAN_H/L 트위스트 페어 여부를 실물로 확인
...
| P16 | **CAN 물리층 신호 품질 불량** | **미해결 (하드웨어)** | `can0` 가 상시 `ERROR-WARNING`. 60초 실측: 에러 프레임 4713개 중 bit-stuffing 958 · frame-format 279 (≈20회/초). `no-ack 0` 이라 송신은 전부 ACK 받고 있고, 피드백도 50Hz 무손실이라 **재송신이 문제를 완전히 가리고 있었다.** 코드로는 못 고친다 → 종단저항 120Ω·스터브 길이·트위스트 페어 실물 점검. **대역폭은 문제가 아니다**: 모터 6개 = 피드백 300f/s + 명령 60f/s ≈ **버스 점유율 5.5%** @1Mbps. 진짜 위험은 `restart-ms 0`(bus-off 시 수동 복구만) |
| P17 | **`ros2 topic pub -1` 이 메시지를 조용히 유실한다** | **해결** (2026-09-03) | `--keep-alive` 기본 0.1초 → RELIABLE 퍼블리셔가 핸드셰이크 전에 파괴됨. 실측 70회 중 1회. `ak45_deg` 를 `-t 3 -r 10 --keep-alive 1.0` 으로 변경 + 노드에 목표각 수신 INFO 추가 |
| P18 | **`onCommandTimer()` S3 게이트가 무로그였다** | **해결** (2026-09-03) | 명령을 받았는데도 피드백이 없어 프레임을 안 보내는 상황이 로그 0줄이었다. "명령이 도달 안 함"과 구분 불가 → WARN 추가 |
